...
def run_all_tests():
    ...
    try:
        ...
        q1 = db.query(ProductionAnnual).filter(
            ProductionAnnual.mine_code == "GEVRA",
            ProductionAnnual.financial_year == "2024-25"
        ).first()
        assert q1 is not None, "Query 1 failed: No production record found for 2024-25"
        results["query_1"] = {
        ...
        seams = db.query(CoalSeam).filter(CoalSeam.mine_code == "GEVRA").order_by(CoalSeam.seam_id).all()
        seam_names = [s.seam_id for s in seams]
        ...
        avg_res = db.execute(text("SELECT AVG(avg_thickness_m) as avg_th, MIN(avg_thickness_m) as min_th, MAX(avg_thickness_m) as max_th FROM coal_seams WHERE mine_code = 'GEVRA'")).mappings().first()
        results["query_3"] = {
        ...
        bh_intervals = db.execute(text("""
            SELECT bi.borehole_id, COUNT(DISTINCT bi.coal_seam) as seams_intersected, MIN(bi.from_depth_m) as min_depth, MAX(bi.to_depth_m) as max_depth
            FROM borehole_intervals bi
            JOIN boreholes_master bm ON bi.borehole_id = bm.borehole_id
            WHERE bm.mine_code = 'GEVRA'
        ...
        doc = db.query(Document).filter(
            Document.mine_code == "GEVRA",
            Document.file_name.like("%202425%")
        ).first()
        assert doc is not None, "Query 5 failed: FY202425 PDF not found in documents"
        ...
        results["query_5"] = {
            ...
            "sample_evidence": chunks[0].chunk_text[:300].replace('\n', ' ') if chunks else "None",
            "status": "PASS"
        }
        ...
        zones = db.query(GeotechnicalZone).filter(GeotechnicalZone.mine_code == "GEVRA").all()
        assert len(zones) == 4, f"Query 6 failed: Expected 4 zones, got {len(zones)}"
        zone_details = [{"zone_id": z.zone_id, "zone_type": z.zone_type, "risk_class": z.risk_class, "basis": z.basis} for z in zones]
        results["query_6"] = {
        ...
        # Query 7: PostGIS Spatial Queries (Borehole distances & nearest features)
        logger.info("--- Query 7: PostGIS Spatial Query ---")
        spatial_check = db.execute(text("""
            SELECT b1.borehole_id as bh1, b2.borehole_id as bh2,
                   ST_Distance(b1.geom::geography, b2.geom::geography) as dist_meters
            FROM spatial_boreholes b1, spatial_boreholes b2
            WHERE b1.mine_code = 'GEVRA' AND b2.mine_code = 'GEVRA'
              AND b1.borehole_id < b2.borehole_id
        ...
        rag_obs = [c.chunk_text[:200].replace('\n', ' ') for c in chunks[:2]]
        results["query_8"] = {
        ...
        user3 = db.query(User).filter(User.user_id == "USR003").first()
        assert user3 is not None, "USR003 not found"
        user3_scopes = db.query(UserScope).filter(UserScope.user_id == user3.user_id).all()
        user3_mine_codes = [s.mine_code for s in user3_scopes]
        
        # Verify USR003 cannot access GEVRA
        has_gevra = "GEVRA" in user3_mine_codes or "GV001" in user3_mine_codes
        ...
        assert not has_gevra, "Security Failure: USR003 has unauthorized access to GEVRA!"
